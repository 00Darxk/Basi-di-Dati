...
\hypersetup{
    colorlinks=true,
    linkcolor=black,
    pdftitle={Esercizi Svolti di Basi di Dati},
    pdfauthor={Giacomo Sturm},
    pdfsubject={Basi di Dati},
    pdfkeywords={Database, Basi di Dati, SQL, RelaX, Entity-Relationship}
}
...
\begin{itemize}
    \item Compagnia(\myuline{Codice}, $\cdots$)
    \item Volo(\myuline{Codice}, \myuline{Numero}, $\cdots$)
\end{itemize}

\subsection{Esercizio 5\footnote{\textit{Dall'esercitazione in aula del 25 Novembre 2024}}}
...
\begin{itemize}
    \item Banca(\myuline{ABI}, $\cdots$)
    \item Agenzia(\myuline{ABI}, \myuline{CAB}, $\cdots$)
    \item CC(\myuline{ABI}, \myuline{CAB}, \myuline{NUM}, $\cdots$)
\end{itemize}

Ci sono alcuni casi nei quali l'identificazione esterna ha un significato particolare e necessario. 

\subsection{Esercizio 6\footnote{\textit{Dall'esercitazione in aula del 25 Novembre 2024}}}
...
\begin{itemize}
    \item Impiegato(\myuline{Codice}, Cognome, Data Afferenza*)
    \item Progetto(\myuline{Nome}, Budget)
    \item Sede(\myuline{Città}, Indirizzo)
    \item Dipartimento(\myuline{Nome}, \myuline{Città}, Telefono, Direttore*)
    \item Partecipazione(\myuline{Progetto}, \myuline{Impiegato})
\end{itemize}

\subsection{Esercizio 7\footnote{\textit{Dall'esercitazione in aula del 25 Novembre 2024}}}
...
\begin{itemize}
    \item Impiegato(\myuline{Codice}, Cognome, Data Afferenza*)
    \item Progetto(\myuline{Nome}, Budget )
    \item Sede(\myuline{Nome Dipartimento}, \myuline{Città}, Indirizzo )
    \item Dipartimento(\myuline{Nome}, Telefono, Direttore)
    \item Partecipazione(\myuline{Progetto}, \myuline{Impiegato})
\end{itemize}

\subsection{Esercizio 8\footnote{\textit{Dall'esercitazione in aula del 25 Novembre 2024}}}
...
\begin{center}
    ...
    \begin{tabular}{|c|c|c|}
        \hline
        \myuline{Nome} & Altitudine & Seggio\\
        \hline
        Castello & 300 & 1\\
        \hline
        Riva & 200 & 2\\
...
\subsection{Esercizio 9\footnote{\textit{Dall'esercitazione in aula del 25 Novembre 2024}}}

Mostrare uno schema concettuale per una realtà i cui dati siano organizzati per mezzo del seguente schema relazionale:
\begin{itemize}
    \item Nazioni(\myuline{Codice}, Nome);
    \item Compagnie(\myuline{Codice}, Nome, Nazione), con un vincolo di integrità referenziale fra l'attributo nazione e la relazione nazioni;
    \item Aeroporti(\myuline{Codice}, Nome, Città), con un vincolo di integrità referenziale fra città e la relazione città;
    \item Città(\myuline{Codice}, Nome);
    \item Voli(\myuline{Compagnia}, \myuline{Numero}, Aeroporto Partenza, Aeroporto Arrivo, Ora Partenza Prevista, Ora Arrivo Previsto, Tipo Aeromobile), con vincoli 
    ...
    \item VoliEffettivi(\myuline{Compagnia}, \myuline{Numero}, \myuline{Data}, Ora Partenza Effettiva, Ora Arrivo Effettivo), un vincolo di integrità referenziale fra compagnia e numero e la relazione voli;
    \item TipoAeroMobile(\myuline{Codice}, Nome).
\end{itemize}
...
\begin{itemize}
    \item Nazione(\myuline{CN}, Nazione);
    \item Città(\myuline{CC},\myuline{CN});
    \item Istituzione(\myuline{Codice}, Nome, Città, Nazione);
    \item Persona(\myuline{Codice}, Nome);
    \item Persona Con Affiliazione(\myuline{Persona}, \myuline{Istituzione})
    \item Sessione(\myuline{Codice}, Titolo, Ora);
    \item Intervento(\myuline{Sessione}, \myuline{Persona}, \myuline{Istituzione});
    \item Convegno(Data, \myuline{Anno}, \myuline{Serie}, CC, CN)
    \item Serie(\myuline{Codice}, Titolo, CM)
    \item Materia(\myuline{CM}, Materia)
\end{itemize}
...
\begin{itemize}
    \item Dipendente(\myuline{Codice Fiscale}, Cognome, Nome);
    \item Professore(\myuline{Codice Fiscale}, Qualifica, Anzianità, Dipartimento*), con vincolo di integrità referenziale fra Codice Fiscale e la relazione Dipendente e fra Dipartimento e la relazione Dipartimento;
    \item Dipartimento(\myuline{Codice}, Nome, Indirizzo);
    \item Corso di Studio(\myuline{Codice}, Nome, Dipartimento, Coordinatore), con vincolo di integrità referenziale fra Dipartimento e la relazione Dipartimento e fra Coordinatore e la relazione Professore;
    \item Collaborazione(\myuline{Corso di Studio}, \myuline{Dipartimento}, \myuline{Professore}, Tipo), con vincolo di integrità referenziale fra Corso di Studio, Dipartimento e la relazione Corso di Studio e fra Professore e la relazione Professore;
    \item Corso(\myuline{Codice}, Materia, Docente), con vincolo di integrità referenziale fra Materia e la relazione Materia e fra Docente e la relazione Professore;
    \item Materia(\myuline{Sigla}, Nome, Semestre);
\end{itemize}
...
\subsection{Esercizio 16\footnote{\textit{Dall'esercitazione in aula del 8 Novembre e 2 Dicembre 2024}}}

In relazione agli schemi precedenti, realizzare un modello relazionale:
\begin{enumerate}
    \item Divisione(\myuline{Nome}, \myuline{Reparto}), Reparto(\myuline{Nome});
    \item Divisione(\myuline{Nome}), Reparto(\myuline{Nome}, Divisione);
    \item Divisione(\myuline{Nome}), Reparto(\myuline{Nome}, \myuline{Divisione});
    \item Divisione(\myuline{Nome}), Reparto(\myuline{Nome}), Appartenenza(\myuline{Divisione}, \myuline{Reparto})
\end{enumerate}
...
\subsection{Esercizio 20\footnote{\textit{Dall'esercitazione in aula del 13 Dicembre 2024}}}

Considerare la relazione seguente:
\begin{figure}[H]%
    \centering%
    \includegraphics[scale=0.85]{relazione_13-12-24.pdf}%
\end{figure}
...
\begin{center}
    $R$(\myuline{IdComp}, NomeComp, \myuline{Volo}, CodDest, Dest, \myuline{NGiorno}, Giorno, Orario, TipoAereo, Costruttore)
\end{center}
...
\begin{figure}[H]%
    \centering%
    \includegraphics{schema_2_13-12-24.pdf}%    
\end{figure}
...
\begin{itemize}
    \item $R_1$(\myuline{IdComp}, NomeComp);
    \item $R_2$(\myuline{IdComp}, \myuline{Volo}, CodDest, Orario);
    \item $R_3$(\myuline{IdComp}, \myuline{Volo}, \myuline{NGiorno}, TipoAereo);
    \item $R_4$(\myuline{CodDest}, Dest);
    \item $R_5$(\myuline{TipoAereo}, Costruttore);
    \item $R_6$(\myuline{NGiorno}, Giorno). 
\end{itemize}
...
\subsection{Esercizio 22\footnote{\textit{Dall'esonero del 6 Dicembre 2024, Compito A; corretto in aula il 16 Dicembre 2024}}}
...
\begin{itemize}
    \item Nazioni(\myuline{Codice}, Nome);
    \item Città(\myuline{Codice}, Nome, Nazione): con vincolo di integrità referenziale fra Nazione e la relazione Nazioni;
    \item Atleti(\myuline{Codice}, Cognome, Nome, LuogoNascita, Cittadinanza): con vincoli di integrità referenziale
    fra LuogoNascita e la relazione Città e fra Cittadinanza e la relazione Nazioni;
    \item DisciplinePraticate(\myuline{Atleta}, \myuline{Disciplina}, OriOlimpici): con vincoli di integrità referenziale fra Atleta e la relazione Atleti e fra Disciplina e la relazione Discipline, dove OriOlimpici indica il numero
    di medaglie d'oro olimpiche vinte dall'atleta nella disciplina;
    \item Discipline(\myuline{Codice}, Nome, Sport): con vincolo di integrità referenziale fra Sport e la relazione
    Sport, un esempio di disciplina sono i 100 metri piani;
    \item Sport(\myuline{Codice}, Nome), un esempio di sport è l'atletica leggera. 
\end{itemize}
